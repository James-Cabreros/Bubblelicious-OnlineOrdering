...
const Category2 = () => {
  ...
  // Fetch only the "Meals" category from the API
  useEffect(() => {
    const fetchMenuItems = async () => {
      try {
        const response = await axios.get("/api/menu-items?category=Meals");
        setMenuItems(response.data);
      } catch (error) {
        console.error("Error fetching meals menu items:", error);
      }
    };
    fetchMenuItems();
  }, []);
  ...
  return (
    <div className="container mt-4">
      <h1 className="mb-4 text-left">Meals:</h1> {/* Updated Title */}
